            ...
            if selected_name:
                ...
                if details:
                    ...
                    if nav_data:
                        ...
                        # Fix: Proper date parsing with dayfirst=True
                        try:
                        ...
                        if st.button("Run Factor Analysis", type="primary", use_container_width=True):
                            ...
                            st.session_state['start_date'] = start_dt  # Store as datetime
                            st.session_state['end_date'] = end_dt      # Store as datetime
...
if 'analyze' in st.session_state and st.session_state['analyze']:
    ...
    with st.spinner("Performing factor analysis..."):
        try:
            ...
            merged_data = fund_monthly.merge(factor_data_period, left_index=True, right_index=True, how='inner')
            ...
            # Fix: Scale data for regression (convert percentages to decimals)
            X_scaled = X.copy()
            X_scaled[factors] = X_scaled[factors] / 100
            y_scaled = y / 100
            ...
            with col2:
                st.markdown("**Period**")
                st.markdown(f"{st.session_state['start_date'].strftime('%d %b %Y')} to {st.session_state['end_date'].strftime('%d %b %Y')}")
            with col3:
            ...
            # Create bar chart - Fix: Use consistent factor names
            ...
            fig.add_trace(go.Bar(
                ...
                y=all_coeffs,
                marker_color=colors,
                text=[f"<b>{coef:.3f} {sig}</b>" for coef, sig in zip(all_coeffs, all_sigs)],
            ...
            # Size factor - Fix: Use correct factor name SMB5
            ...
            # Actual vs Predicted - Fix: Scale predictions back to percentages
            st.header("Actual vs Predicted Excess Returns")

            merged_data['predicted'] = model.predict(X_scaled) * 100  # Convert back to percentages
